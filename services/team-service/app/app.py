...
async def update_team_by_id(
    teamId: uuid.UUID, team: TeamUpdate, db: Session = Depends(get_db)
):
...
@app.patch(
    "/teams/{teamId}/add-users/{userId}",
    tags=["teams"],
    response_model=TeamRead,
)
async def add_user_in_team(
    teamId: uuid.UUID, userId: uuid.UUID, db: Session = Depends(get_db)
):
...
@app.patch(
    "/teams/{teamId}/delete-users/{userId}",
    tags=["teams"],
    response_model=TeamRead,
)
async def delete_user_from_team(
    ...
    remove_user_from_team = crud.delete_user_team(
        teamId=teamId, userId=userId, db=db
    )
