...
async def get_match(
    matchId: uuid.UUID, db: Session = Depends(get_db)
) -> MatchRead:
...
async def update_match(
    matchId: uuid.UUID, match: MatchUpdate, db: Session = Depends(get_db)
) -> MatchUpdate:
...
async def delete_match(
    ...
    if crud.delete_match(matchId, db):
        return JSONResponse(
            status_code=200, content={"message": "Match successfully deleted"}
        )
    return JSONResponse(status_code=404, content={"message": "Match not found"})
